...
productsRouter.post('/', requireAdmin, async (req: Request, res: Response, next) => {
  try {
    ...
    if (data.barcode) {
      const existingBarcode = await prisma.product.findFirst({ where: { barcode: data.barcode } });
      if (existingBarcode) throw new ConflictError(`Product with barcode "${data.barcode}" already exists`);
    }
