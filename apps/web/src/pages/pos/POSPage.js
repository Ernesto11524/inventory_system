...
import { Search, Plus, Minus, Trash2, ShoppingCart, Barcode, CheckCircle, X, ReceiptText, Tag, User, Phone, CreditCard, Banknote, History, ChevronRight, Printer, } from 'lucide-react';
...
function ProductCard({ product, onAdd }) {
    ...
    return (<button onClick={() => !disabled && onAdd(product)} disabled={disabled} className={clsx('text-left p-3 rounded-xl border-2 transition-all w-full', disabled
      ...
      <div className="flex items-center justify-between">
        ...
        <span className={clsx('text-[10px] font-medium px-1.5 py-0.5 rounded-full', isOut ? 'bg-red-100 text-red-600' :
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm">
        <div className="p-6">
          ...
          {sale.customerName && (<div className="bg-gray-50 rounded-lg p-2.5 mb-3 text-xs">
          ...
          <div className="border-t border-dashed border-gray-300 pt-3 mb-3 space-y-2 max-h-40 overflow-y-auto">
            {sale.items.map((item) => (<div key={item.id} className="flex justify-between text-xs">
                <div>
          ...
          <div className="border-t border-dashed border-gray-300 pt-3 space-y-1.5 text-sm">
            ...
            {sale.discount > 0 && (<div className="flex justify-between text-green-600">
                <span>Discount</span>
            ...
            {sale.change > 0 && (<div className="flex justify-between text-xs font-medium text-green-600">
                <span>Change</span>
...
function SaleHistoryPanel({ onClose }) {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: () => get('/sales', { from, to, limit: 50 }),
    });
    const sales = data?.data || [];
    const totalRevenue = sales.reduce((s, sale) => s + Number(sale.total), 0);
    return (<div className="fixed inset-0 z-40 flex">
      ...
      <div className="relative ml-auto w-full max-w-lg bg-white h-full flex flex-col shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto">
          {isLoading ? <LoadingSpinner className="h-32"/> : sales.length === 0 ? (<div className="text-center py-16 text-gray-400">
              <ReceiptText size={32} className="mx-auto mb-2 opacity-50"/>
              <p>No sales found</p>
            </div>) : (<div className="divide-y divide-gray-100">
              {sales.map((sale) => (<div key={sale.id}>
                  <button onClick={() => setSelected(selected?.id === sale.id ? null : sale)} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-50 text-left">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex gap-2 mt-0.5">
                        <span className={clsx('text-[10px] font-medium px-1.5 py-0.5 rounded-full', sale.paymentMethod === 'cash' ? 'bg-green-100 text-green-700' :
                  ...
                  {selected?.id === sale.id && (<div className="px-4 pb-3 bg-gray-50 border-t border-gray-100">
                      <div className="space-y-1.5 mt-2">
                        ...
                        {sale.discount > 0 && (<div className="flex justify-between text-xs text-green-600">
                        ...
                        <button onClick={() => setReprinting(sale)} className="mt-3 w-full flex items-center justify-center gap-2 bg-brand-600 text-white py-2 rounded-lg text-xs font-semibold hover:bg-brand-700 transition-colors">
      ...
      {reprinting && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm">
            <div className="p-6">
              ...
              {reprinting.customerName && (<div className="bg-gray-50 rounded-lg p-2.5 mb-3 text-xs">
              ...
              <div className="border-t border-dashed border-gray-300 pt-3 mb-3 space-y-2 max-h-48 overflow-y-auto">
                {reprinting.items.map((item) => (<div key={item.id} className="flex justify-between text-xs">
                    <div>
                      <p className="font-medium text-gray-900">{item.product?.name}</p>
                      <p className="text-gray-400">{item.quantity} × GH₵{Number(item.unitPrice).toFixed(2)}</p>
                    </div>
              ...
              <div className="border-t border-dashed border-gray-300 pt-3 space-y-1.5 text-sm">
                ...
                {reprinting.discount > 0 && (<div className="flex justify-between text-green-600">
                    <span>Discount</span>
                ...
                {reprinting.change > 0 && (<div className="flex justify-between text-xs font-medium text-green-600">
                    <span>Change</span>
...
export function POSPage() {
    ...
    const { data: productsData, isLoading } = useQuery({
        ...
        queryFn: () => get('/products', { limit: 100, search: search || undefined, categoryId: categoryId || undefined }),
    });
    ...
    const { data: summaryData } = useQuery({
        ...
        queryFn: () => get('/sales/summary/today'),
    ...
    const updateQty = (productId, delta) => {
        setCart((prev) => prev.map((i) => {
            ...
            if (newQty <= 0)
                return i;
            if (newQty > i.currentStock) {
                toast.error(`Only ${i.currentStock} in stock`);
                return i;
            }
    ...
    const handleBarcodeScan = async (e) => {
        ...
        setBarcodeInput('');
        try {
            const res = await get(`/products/barcode/${barcode}`);
            if (res.data)
                addToCart(res.data);
        }
        catch {
    ...
    return (<div className="flex h-[calc(100vh-4rem)] -m-4 lg:-m-6 overflow-hidden">
      {/* Left — Products */}
      <div className="flex-1 flex flex-col bg-gray-50 overflow-hidden border-r border-gray-200">
        <div className="p-3 bg-white border-b border-gray-200 space-y-2 shrink-0">
          {summary && (<div className="flex gap-3 text-xs bg-brand-50 rounded-lg px-3 py-2 items-center">
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input ref={barcodeRef} value={barcodeInput} onChange={e => setBarcodeInput(e.target.value)} onKeyDown={handleBarcodeScan} className="input pl-8 text-sm font-mono" placeholder="Scan barcode…" autoFocus/>
            </div>
            <div className="relative flex-1">
            ...
            <button onClick={() => setShowAddItem(true)} className="shrink-0 bg-brand-600 text-white px-4 rounded-xl font-semibold text-sm hover:bg-brand-700 flex items-center gap-1.5 transition-colors">
              <Plus size={14}/> Add
            </button>
          </div>
          ...
          <div className="flex gap-1.5 overflow-x-auto pb-0.5">
            ...
            {categories.map((c) => (<button key={c.id} onClick={() => setCategoryId(c.id)} className={clsx('shrink-0 px-3 py-1 rounded-full text-xs font-medium', categoryId === c.id ? 'bg-brand-600 text-white' : 'bg-gray-100 text-gray-600')}>
                {c.name}
              </button>))}
          </div>
        </div>
        <div className="flex-1 overflow-y-auto p-3">
          {isLoading ? <LoadingSpinner className="h-64"/> : (<div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2">
      ...
      <div className="w-80 xl:w-96 flex flex-col bg-white shrink-0 overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-200 shrink-0 flex items-center justify-between">
          ...
          {cart.length > 0 && (<button onClick={() => { setCart([]); setDiscount(0); }} className="text-xs text-red-400 hover:text-red-600 flex items-center gap-1">
              <X size={11}/> Clear
            </button>)}
        </div>

        <div className="flex-1 overflow-y-auto">
          {cart.length === 0 ? (<div className="text-center py-10 text-gray-400">
              ...
              {cart.map((item) => (<div key={item.productId} className="px-4 py-2.5">
          ...
          <div className="border-t border-gray-200 px-4 py-3">
            ...
            <div className="flex flex-col gap-1.5">
              {PAYMENT_METHODS.map(({ key, label, icon: Icon, color, desc }) => (<button key={key} onClick={() => setPaymentMethod(key)} className={clsx('flex items-center gap-3 px-3 py-2.5 rounded-lg border-2 text-sm font-medium transition-all w-full', paymentMethod === key ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-gray-200 text-gray-600 hover:border-gray-300')}>
        ...
        <div className="border-t border-gray-200 px-4 py-3 shrink-0 space-y-2">
          ...
          {(paymentMethod === 'cash') && (<div className="flex items-center gap-2">
          ...
          <div className="bg-gray-50 rounded-xl p-3 space-y-1.5">
            ...
            {paymentMethod === 'cash' && amountPaid !== '' && Number(amountPaid) >= total && (<div className="flex justify-between text-sm font-semibold text-green-600"><span>Change</span><span>GH₵{change.toFixed(2)}</span></div>)}
          </div>
          <button onClick={() => saleMutation.mutate()} disabled={cart.length === 0 || saleMutation.isPending} className={clsx('w-full py-3 rounded-xl font-bold text-sm transition-all', cart.length === 0 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-brand-600 text-white hover:bg-brand-700 active:scale-95 shadow-lg')}>
      ...
      {/* Add Item Modal */}
      {showAddItem && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            ...
            <div className="p-4">
              <div className="relative mb-4">
                ...
                <input autoFocus className="input pl-9" placeholder="Search by name, SKU or barcode…" value={addItemSearch} onChange={e => setAddItemSearch(e.target.value)}/>
              </div>
              <div className="max-h-80 overflow-y-auto space-y-1">
                {products
                .filter((p) => !addItemSearch || p.name.toLowerCase().includes(addItemSearch.toLowerCase()) || p.sku.toLowerCase().includes(addItemSearch.toLowerCase()))
                .map((product) => {
                const stock = product.inventory?.currentStock ?? 0;
                const isOut = stock <= 0;
                return (<button key={product.id} disabled={isOut} onClick={() => { addToCart(product); setShowAddItem(false); setAddItemSearch(''); }} className={clsx('w-full flex items-center gap-3 px-4 py-3 rounded-xl border transition-all text-left', isOut ? 'border-gray-100 bg-gray-50 opacity-50 cursor-not-allowed' : 'border-gray-200 hover:border-brand-400 hover:bg-brand-50 cursor-pointer')}>
...
//# sourceMappingURL=POSPage.js.map