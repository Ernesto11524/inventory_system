...
import {
  Plus, Search, Upload, Edit2, Trash2,
  Package, ChevronRight, Loader2, ScanLine,
} from 'lucide-react';
...
import { productSchema, type ProductInput } from '@inventory/shared';
...
function StockBadge({ stock, min }: { stock?: number; min: number }) {
  if (stock === undefined) return <span className="badge-gray">No data</span>;
  if (stock <= 0) return <span className="badge-red">Out of stock</span>;
  if (stock < min) return <span className="badge-yellow">{stock} (low)</span>;
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <div className="relative">
            ...
            {lookingUp ? (
              ...
              <ScanLine size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400" />
            )}
          </div>
        ...
        <div>
          ...
          <select {...register('unit')} className="input">
            {['pcs', 'kg', 'g', 'l', 'ml', 'box', 'pack', 'bag', 'can', 'bottle', 'roll', 'pair', 'set'].map(u => (
...
export function ProductsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => get<Product[]>('/products', { page, limit: 20, search, categoryId }),
  });
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {products.map((product) => (
                  <tr key={product.id} className="hover:bg-gray-50 cursor-pointer transition-colors" onClick={() => navigate(`/products/${product.id}`)}>
