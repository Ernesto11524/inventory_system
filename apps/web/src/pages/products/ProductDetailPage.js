...
import { ArrowLeft, Package, PlusCircle, TrendingUp, TrendingDown, RefreshCw, RotateCcw, Wrench, } from 'lucide-react';
import { format } from 'date-fns';
import toast from 'react-hot-toast';
import { stockEntrySchema } from '@inventory/shared';
...
function StockEntryForm({ productId, onClose, isAdmin }) {
    ...
    const mutation = useMutation({
        mutationFn: (data) => post('/stock/entry', { ...data, productId }),
        onSuccess: () => {
        ...
        onError: (err) => toast.error(err.response?.data?.message || 'Failed to record entry'),
    });
    return (<form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-4">
      ...
      <div>
        ...
        <input {...register('quantity', { valueAsNumber: true })} type="number" min="1" className="input" placeholder="1"/>
...
export function ProductDetailPage() {
    ...
    const { data: historyData } = useQuery({
        ...
        queryFn: () => get(`/stock/history/${id}`, { page: historyPage, limit: 15 }),
        enabled: !!id,
    });
    if (isLoading)
    ...
    return (<div className="animate-fade-in space-y-6">
      ...
      <div>
        ...
        <div className="flex items-start gap-5">
          <div className="w-20 h-20 rounded-xl bg-gray-100 overflow-hidden shrink-0">
            {product.imageUrl ? (<img src={product.imageUrl} alt={product.name} className="w-full h-full object-cover"/>) : (<div className="w-full h-full flex items-center justify-center">
          ...
          <div className="flex-1">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                {product.category && (<span className="badge-blue mt-1 inline-flex">{product.category.name}</span>)}
              </div>
              <button onClick={() => setShowStockForm(true)} className="btn-primary btn-sm">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        ].map(({ label, value, color }) => (<div key={label} className="card p-4">
      ...
      {/* Product details */}
      {product.description && (<div className="card p-5">
      ...
      <div className="card overflow-hidden">
        ...
        {history.length === 0 ? (<div className="py-12 text-center text-sm text-gray-400">No stock entries yet</div>) : (<table className="w-full text-sm">
            <thead>
