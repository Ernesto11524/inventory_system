...
function SupplierForm({ supplier, onClose }) {
    ...
    const mutation = useMutation({
        mutationFn: (data) => isEditing ? put(`/suppliers/${supplier.id}`, data) : post('/suppliers', data),
        onSuccess: () => {
...
export function SuppliersPage() {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: () => get('/suppliers', { page, limit: 20 }),
    });
    ...
    return (<div className="animate-fade-in">
      <PageHeader title="Suppliers" actions={isAdmin ? (<button onClick={() => { setEditSupplier(null); setShowForm(true); }} className="btn-primary btn-sm">
      ...
      {isLoading ? (<LoadingSpinner className="h-64"/>) : (<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {suppliers.length === 0 ? (<div className="col-span-3">
              <EmptyState message="No suppliers yet" icon="product" className="h-64"/>
            </div>) : (suppliers.map((s) => (<div key={s.id} className="card p-5 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-3">
                  ...
                  {isAdmin && (<div className="flex gap-1">
                      ...
                      <button onClick={() => { if (confirm(`Delete "${s.name}"?`))
                    deleteMutation.mutate(s.id); }} className="btn-ghost btn-sm p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50">
                        <Trash2 size={13}/>
                      </button>
                    </div>)}
                </div>
                ...
                <div className="mt-3 space-y-1.5">
                  {s.email && (<div className="flex items-center gap-2 text-xs text-gray-500">
                      <Mail size={11}/> {s.email}
                    </div>)}
                  {s.phone && (<div className="flex items-center gap-2 text-xs text-gray-500">
                      <Phone size={11}/> {s.phone}
                    </div>)}
                  {s.address && (<div className="flex items-start gap-2 text-xs text-gray-500">
      ...
      <Modal isOpen={showForm} onClose={() => { setShowForm(false); setEditSupplier(null); }} title={editSupplier ? 'Edit Supplier' : 'New Supplier'}>
