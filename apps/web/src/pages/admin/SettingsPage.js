...
import { Store, CreditCard, Palette, Save, Eye, EyeOff, Sun, Moon, Building2, Phone, Mail, MapPin, Tag, Check, } from 'lucide-react';
...
function Section({ icon: Icon, title, subtitle, children, }) {
    return (<div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
...
function ThemeToggle() {
    ...
    return (<div className="flex gap-3">
      {['light', 'dark'].map((t) => (<button key={t} onClick={() => setTheme(t)} className={clsx('flex-1 flex items-center justify-center gap-2.5 py-3 px-4 rounded-xl border-2 text-sm font-medium transition-all', theme === t
...
export function SettingsPage() {
    ...
    const mutation = useMutation({
        mutationFn: (body) => patch('/settings', body),
        onSuccess: (res, vars) => {
            const s = res.data;
            setSettings({
    ...
    const savePayment = paymentForm.handleSubmit((values) => {
        const body = {};
        if (values.paystackPublicKey)
            body.paystackPublicKey = values.paystackPublicKey;
        if (values.paystackSecretKey)
            body.paystackSecretKey = values.paystackSecretKey;
        ...
        <form onSubmit={saveStore} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Store Name" icon={Store} error={storeForm.formState.errors.storeName?.message}>
              <input className="input dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 dark:placeholder:text-gray-500" placeholder="e.g. Kofi's Supermarket" {...storeForm.register('storeName', { required: 'Store name is required' })}/>
            </Field>
            <Field label="Tagline" icon={Tag}>
              <input className="input dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 dark:placeholder:text-gray-500" placeholder="e.g. Fresh & Quality Every Day" {...storeForm.register('storeTagline')}/>
            </Field>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Email" icon={Mail}>
              <input type="email" className="input dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 dark:placeholder:text-gray-500" placeholder="store@example.com" {...storeForm.register('storeEmail')}/>
            </Field>
            <Field label="Phone" icon={Phone}>
              <input className="input dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 dark:placeholder:text-gray-500" placeholder="+233 XX XXX XXXX" {...storeForm.register('storePhone')}/>
            </Field>
          </div>
          <Field label="Address" icon={MapPin}>
            <input className="input dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 dark:placeholder:text-gray-500" placeholder="Street, City, Region" {...storeForm.register('storeAddress')}/>
          </Field>
          <div className="pt-2 flex justify-end">
            <button type="submit" disabled={isSaving} className={clsx('btn-primary btn-sm flex items-center gap-2', savedSection === 'store' && 'bg-green-600 hover:bg-green-700')}>
      ...
      <Section icon={CreditCard} title="Payment Settings" subtitle="Paystack keys used for MoMo, card, and bank payments in POS">
        <form onSubmit={savePayment} className="space-y-4">
          ...
          <Field label="Paystack Public Key">
            <input className="input font-mono text-xs dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 dark:placeholder:text-gray-500" placeholder="pk_live_... or pk_test_..." {...paymentForm.register('paystackPublicKey')}/>
          </Field>

          <Field label={`Paystack Secret Key${serverSettings?.hasSecretKey ? ' (already set — leave blank to keep)' : ''}`}>
            <div className="relative">
              <input type={showSecretKey ? 'text' : 'password'} className="input font-mono text-xs pr-10 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 dark:placeholder:text-gray-500" placeholder={serverSettings?.hasSecretKey ? '••••••••••••••••' : 'sk_live_... or sk_test_...'} {...paymentForm.register('paystackSecretKey')}/>
              <button type="button" onClick={() => setShowSecretKey((v) => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
          ...
          <div className="pt-2 flex justify-end">
            <button type="submit" disabled={isSaving} className={clsx('btn-primary btn-sm flex items-center gap-2', savedSection === 'payment' && 'bg-green-600 hover:bg-green-700')}>
