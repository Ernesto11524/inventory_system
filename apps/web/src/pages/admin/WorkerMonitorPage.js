...
import { Users, LogIn, ShoppingCart, Activity, UserPlus, Eye, EyeOff, } from 'lucide-react';
import toast from 'react-hot-toast';
import { get, post } from '../../utils/api';
import { PageHeader, LoadingSpinner, Modal } from '../../components/ui/index';
import clsx from 'clsx';
function StatBadge({ value, label, color }) {
    return (<div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${color}`}>
...
function WorkerCard({ worker, onViewDetails }) {
    ...
    return (<div className="card p-5">
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <div className={clsx('absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white', isOnline ? 'bg-green-500' : 'bg-gray-300')}/>
          </div>
          <div>
            <p className="font-semibold text-gray-900">{worker.user.name}</p>
            <p className="text-xs text-gray-500">{worker.user.email}</p>
            <span className={clsx('text-[10px] font-medium px-1.5 py-0.5 rounded-full', worker.user.role === 'admin' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700')}>
              {worker.user.role}
            </span>
          </div>
        </div>
        <div className="flex flex-col items-end gap-1">
          ...
          {worker.lastSeen && (<p className="text-xs text-gray-400">
      ...
      {/* Login times */}
      {worker.logins.length > 0 && (<div className="mb-4 p-3 bg-gray-50 rounded-lg">
          <p className="text-xs font-medium text-gray-600 mb-1.5">Login Times Today</p>
          <div className="flex flex-wrap gap-1.5">
            {worker.logins.slice(0, 5).map((t, i) => (<span key={i} className="text-xs bg-white border border-gray-200 rounded px-2 py-0.5 font-mono">
      ...
      <div className="flex gap-2">
        <button onClick={() => onViewDetails(worker)} className="btn-secondary btn-sm flex-1 gap-1">
...
function AddUserModal({ onClose }) {
    ...
    return (<div className="space-y-4">
      ...
      <div>
        ...
        <div className="relative">
          <input className="input pr-10" type={showPass ? 'text' : 'password'} placeholder="Min 6 characters" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })}/>
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        <button onClick={() => mutation.mutate()} disabled={mutation.isPending || !form.name || !form.email || !form.password} className="btn-primary">
          {mutation.isPending ? 'Creating…' : 'Create User'}
        </button>
      </div>
    </div>);
}
function ActivityModal({ worker, onClose }) {
    const [from, setFrom] = useState(format(subDays(new Date(), 7), 'yyyy-MM-dd'));
    const [to, setTo] = useState(format(new Date(), 'yyyy-MM-dd'));
    const { data, isLoading } = useQuery({
        queryKey: ['worker-activity', worker.user.id, from, to],
        queryFn: () => get(`/activity/users/${worker.user.id}`, {
            from,
            to,
            limit: 100,
        }),
    ...
    return (<div>
      ...
      {isLoading ? (<LoadingSpinner className="h-32"/>) : logs.length === 0 ? (<p className="text-center text-gray-400 py-8">No activity in this period</p>) : (<div className="space-y-2 max-h-80 overflow-y-auto">
          {logs.map((log) => (<div key={log.id} className="flex items-center gap-3 p-2.5 bg-gray-50 rounded-lg">
...
export function WorkerMonitorPage() {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: () => get('/activity/workers', { from: date, to: date }),
        refetchInterval: 60000,
    });
    const workers = data?.data || [];
    const activeWorkers = workers.filter((w) => w.logins.length > 0);
    return (<div className="animate-fade-in">
      <PageHeader title="Worker Monitor" subtitle="Track staff activity, logins, and sales" actions={<div className="flex gap-2">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
            ...
            { label: 'Total Sales Today', value: workers.reduce((s, w) => s + w.salesCount, 0), icon: ShoppingCart, color: 'bg-amber-500' },
            { label: 'Total Logins', value: workers.reduce((s, w) => s + w.logins.length, 0), icon: LogIn, color: 'bg-purple-500' },
        ].map(({ label, value, icon: Icon, color }) => (<div key={label} className="card p-4 flex items-center gap-3">
      ...
      {isLoading ? (<LoadingSpinner className="h-64"/>) : (<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {workers.map((worker) => (<WorkerCard key={worker.user.id} worker={worker} onViewDetails={setSelectedWorker}/>))}
        </div>)}
      ...
      <Modal isOpen={!!selectedWorker} onClose={() => setSelectedWorker(null)} title="Worker Activity Log" size="lg">
        {selectedWorker && (<ActivityModal worker={selectedWorker} onClose={() => setSelectedWorker(null)}/>)}
      </Modal>
    </div>);
}
//# sourceMappingURL=WorkerMonitorPage.js.map