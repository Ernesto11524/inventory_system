...
import { purchaseOrderSchema, type PurchaseOrderInput } from '@inventory/shared';
...
function POForm({ onClose }: { onClose: () => void }) {
  ...
  const { data: suppliersData } = useQuery({
    ...
    queryFn: () => get<any[]>('/suppliers', { limit: 100 }),
  });

  const { data: productsData } = useQuery({
    queryKey: ['products-all'],
    queryFn: () => get<any[]>('/products', { limit: 200 }),
  });

  const { register, handleSubmit, control, formState: { errors } } = useForm<PurchaseOrderInput>({
  ...
  const mutation = useMutation({
    mutationFn: (data: PurchaseOrderInput) => post('/purchase-orders', data),
    onSuccess: () => {
        ...
        <div className="space-y-2">
          {fields.map((field, idx) => (
            <div key={field.id} className="flex gap-2 items-center">
              ...
              />
              {fields.length > 1 && (
                <button type="button" onClick={() => remove(idx)} className="btn-ghost btn-sm p-1.5 text-red-400 hover:text-red-600">
                  <Trash2 size={13} />
                </button>
              )}
            </div>
...
export function PurchaseOrdersPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => get<any[]>('/purchase-orders', { page, limit: 20, ...(statusFilter ? { status: statusFilter } : {}) }),
  });
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        ...
        subtitle={`${pagination?.total ?? '—'} orders`}
        actions={isAdmin ? (
      ...
      <div className="flex gap-2 mb-4">
        {[
          ...
          <button
            ...
            className={clsx(
              'btn btn-sm px-3',
              statusFilter === value ? 'bg-brand-600 text-white' : 'btn-secondary',
            )}
          >
            {label}
          </button>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {orders.map((order: any) => {
                const totalCost = order.items?.reduce((sum: number, i: any) => sum + (i.quantity * Number(i.unitCost)), 0) ?? 0;
                return (
                  <tr
                    key={order.id}
