...
function MetricCard({ label, value, sub, icon: Icon, color, positive }: {
  label: string; value: string; sub?: string; icon: any; color: string; positive?: boolean;
}) {
  return (
...
export function SalesReportPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => get<any>('/reports/sales', { from: fromISO, to: toISO }),
    enabled: !!from && !!to,
  });
  ...
  const dailyData = (report?.dailyBreakdown || []).map((d: any) => ({
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="Sales Report"
        subtitle="Revenue, cost of goods sold, and profit"
        actions={
      ...
      <div className="card p-4 mb-6 flex flex-wrap items-end gap-4">
        <div className="flex bg-gray-100 rounded-lg p-0.5 gap-0.5">
          {[{ key: 'week', label: 'This Week' }, { key: 'month', label: 'This Month' }, { key: 'custom', label: 'Custom' }].map(({ key, label }) => (
            <button key={key} onClick={() => handlePeriodChange(key as Period)}
              className={clsx('px-4 py-1.5 text-sm font-medium rounded-md transition-colors',
                period === key ? 'bg-white shadow text-brand-700' : 'text-gray-600 hover:text-gray-900')}>
              {label}
            </button>
          ))}
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 mb-6">
            <div className="card p-5">
              ...
              {dailyData.length === 0 ? <EmptyState message="No sales in this period" icon="chart" className="h-48" /> : (
                <ResponsiveContainer width="100%" height={240}>
                  <BarChart data={dailyData} margin={{ top: 4, right: 4, bottom: 0, left: -10 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                    <XAxis dataKey="date" tick={{ fontSize: 11, fill: '#9ca3af' }} />
                    <YAxis tick={{ fontSize: 11, fill: '#9ca3af' }} tickFormatter={(v) => `₵${(v/1000).toFixed(0)}k`} />
                    <Tooltip formatter={(v: any) => [`GH₵${Number(v).toFixed(2)}`]} contentStyle={{ borderRadius: 8, border: '1px solid #e5e7eb', fontSize: 12 }} />
          ...
          <div className="card overflow-hidden mb-6">
            ...
            {(report?.topProducts || []).length === 0 ? (
              <EmptyState message="No sales in this period" icon="product" className="h-48" />
            ) : (
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    {['#', 'Product', 'Units Sold', 'Revenue', 'Cost', 'Profit', 'Margin'].map((h, i) => (
                      <th key={h} className={`py-3 px-5 font-medium text-gray-500 text-xs uppercase ${i >= 2 ? 'text-right' : 'text-left'}`}>{h}</th>
                    ))}
                  </tr>
                </thead>
                      ...
                      <tr key={p.productId} className="hover:bg-gray-50">
                        ...
                        <td className="py-3 px-5 text-right">
                          <span className={clsx('text-xs font-medium px-2 py-0.5 rounded-full',
                            Number(margin) >= 20 ? 'bg-green-100 text-green-700' : Number(margin) >= 10 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700')}>
                            {margin}%
                          </span>
                        </td>
                      </tr>
          ...
          <div className="card overflow-hidden">
            ...
            {(report?.transactions || []).length === 0 ? (
              <EmptyState message="No transactions in this period" icon="chart" className="h-48" />
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200">
                      {['Date', 'Product', 'Type', 'Qty', 'Unit Price', 'Total Value', 'By'].map((h, i) => (
                        <th key={h} className={`py-3 px-5 font-medium text-gray-500 text-xs uppercase ${[3,4,5].includes(i) ? 'text-right' : i === 2 ? 'text-center' : 'text-left'}`}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                      ...
                      return (
                        <tr key={t.id} className="hover:bg-gray-50">
                          ...
                          <td className="py-2.5 px-5 text-center">
                            <span className={clsx('text-xs font-medium px-2 py-0.5 rounded-full',
                              isSale ? 'bg-amber-100 text-amber-700' : t.type === 'restock' ? 'bg-green-100 text-green-700' : t.type === 'return' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700')}>
                              {t.type}
                            </span>
                          </td>
