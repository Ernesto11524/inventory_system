...
function StockBar({ current, min }) {
    ...
    return (<div className="w-24 h-1.5 bg-gray-200 rounded-full overflow-hidden">
...
export function InventoryPage() {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: () => tab === 'low'
            ...
            : get('/inventory', { page, limit: 20, search }),
    ...
    return (<div className="animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        ...
        ].map(({ key, label }) => (<button key={key} onClick={() => { setTab(key); setPage(1); }} className={clsx('px-4 py-1.5 text-sm font-medium rounded-md transition-colors', tab === key ? 'bg-brand-600 text-white' : 'text-gray-600 hover:text-gray-900')}>
              {label}
            </button>))}
        </div>
        {tab === 'all' && (<div className="relative flex-1 max-w-xs">
      ...
      <div className="card overflow-hidden">
        {isLoading ? (<LoadingSpinner className="h-64"/>) : items.length === 0 ? (<EmptyState message={tab === 'low' ? 'No low stock items — great!' : 'No inventory data yet'} icon="product" className="h-64"/>) : (<div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                ...
                return (<tr key={item.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => navigate(`/products/${item.productId || product.id}`)}>
                      ...
                      <td className="py-3 px-4 text-right">
                        {isOut ? (<span className="badge-red">Out of stock</span>) : isLow ? (<span className="badge-yellow">Low stock</span>) : (<span className="badge-green">OK</span>)}
                      </td>
                    </tr>);
            })}
              </tbody>
        ...
        {pagination && tab === 'all' && (<div className="px-4">
