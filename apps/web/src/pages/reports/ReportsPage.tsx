...
export function ReportsPage() {
  const [tab, setTab] = useState<'stock-value' | 'movement'>('stock-value');
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="Reports"
        subtitle="Export and analyze inventory data"
        actions={
          <button
            onClick={() => handleExport(tab === 'stock-value' ? 'inventory' : 'movement')}
      ...
      <div className="flex gap-2 mb-4">
        <button
          ...
          className={clsx('btn btn-sm gap-1.5', tab === 'stock-value' ? 'btn-primary' : 'btn-secondary')}
        >
          <BarChart3 size={13} /> Stock Value
        </button>
        <button
          onClick={() => setTab('movement')}
          className={clsx('btn btn-sm gap-1.5', tab === 'movement' ? 'btn-primary' : 'btn-secondary')}
        >
      ...
      {tab === 'movement' && (
        <>
          ...
          {mvLoading ? <LoadingSpinner className="h-64" /> : (
            <div className="card overflow-hidden">
              {movements.length === 0 ? (
                <EmptyState message="No stock movements in this date range" icon="chart" className="h-64" />
              ) : (
                <>
                  <table className="w-full text-sm">
                    <thead>
