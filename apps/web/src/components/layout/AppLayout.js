...
import { LayoutDashboard, Package, Warehouse, Bell, Truck, ShoppingCart, BarChart3, Menu, X, LogOut, Tag, TrendingUp, Monitor, UserCheck, Wifi, WifiOff, Settings, } from 'lucide-react';
...
export function AppLayout() {
    ...
    const { data: alertCount } = useQuery({
        ...
        queryFn: () => get('/alerts/unresolved-count'),
    ...
    return (<div className="flex h-screen bg-gray-50 dark:bg-gray-900 overflow-hidden">
      ...
      {/* Sidebar */}
      <aside className={clsx('fixed inset-y-0 left-0 z-30 flex flex-col w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 transition-transform duration-200 lg:static lg:translate-x-0', sidebarOpen ? 'translate-x-0' : '-translate-x-full')}>
        {/* Logo / Store name */}
        <div className="flex items-center gap-3 px-5 h-16 border-b border-gray-200 dark:border-gray-700 shrink-0">
          ...
          <button onClick={() => setSidebarOpen(false)} className="ml-auto lg:hidden text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 shrink-0">
            <X size={18}/>
          </button>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto p-3 space-y-0.5">
          {navItems.map(({ to, icon: Icon, label, badge }) => (<NavLink key={to} to={to} onClick={() => setSidebarOpen(false)} className={({ isActive }) => clsx('flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors', isActive
                ? 'bg-brand-50 dark:bg-brand-900/40 text-brand-700 dark:text-brand-300'
                : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-gray-100')}>
              <Icon size={16}/>
              <span className="flex-1">{label}</span>
              {badge && unresolvedCount > 0 && (<span className="min-w-[18px] h-[18px] flex items-center justify-center rounded-full bg-red-500 text-white text-[10px] font-bold px-1">
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 p-3 shrink-0">
          <div className="flex items-center gap-3 px-2 py-2 mb-1">
            ...
            <div className="flex items-center gap-1">
              {connected ? (<span title="Real-time connected"><Wifi size={12} className="text-green-500"/></span>) : (<span title="Disconnected"><WifiOff size={12} className="text-gray-400"/></span>)}
            </div>
          </div>
          <button onClick={handleLogout} className="flex items-center gap-2 w-full px-3 py-2 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-red-600 dark:hover:text-red-400 rounded-lg transition-colors">
            <LogOut size={14}/>
            Sign out
          </button>
        </div>
